...
IO::Handle::Moose - Moose reimplementation of IO::Handle with improvements
...
# Constant for IO::Handle::Moose::* package name
...
sub sync {
    ...
    if (defined &IO::Handle::Moose::Syscall::SYS_fsync and defined CORE::fileno $hashref->{fh}) {
        return syscall(&IO::Handle::Moose::Syscall::SYS_fsync, CORE::fileno $hashref->{fh}) == 0
               ? '0 but true'
               : undef;
    }
    else {
...
package IO::Handle::Moose::Tie;

use Moose;

use Scalar::Util 'weaken';

*Debug = \$IO::Handle::Moose::Debug;


# Tie hook. Others are initialized by IO::Handle::Moose.
sub TIEHANDLE {
...
package IO::Handle::Moose::Syscall;
...
called as B<IO::Handle::Moose-E<gt>accessor>.
...
Creates the B<IO::Handle::Moose> object and calls B<fdopen> method if the
I<fd> parameter is defined.

  $io = new IO::Handle::Moose fd=>\*STDIN, mode=>"r";

The object can be created with uninitialized file handle.

  $in = new IO::Handle::Moose;
  $in->fdopen(\*STDIN);

=item new_from_fd(I<fd> [, I<mode>])

Creates the B<IO::Handle::Moose> object and immediately opens the file handle
based on arguments.

  $out = new_from_fd IO::Handle::Moose \*STDOUT, "w";
...
object or file descriptor number.
  ...
  $out = new IO::Handle::Moose;
  $out->fdopen(\*STDOUT, "w");

  $dup = new IO::Handle::Moose;
  $dup->fdopen($out, "a");
...
throws exception on failure which can be caught with try/catch:
  ...
  try eval {
    open $f, "/etc/hostname";
    $io = new IO::Handle::Moose fd=>$f, mode=>"r";
    $c = $io->getc;
  };
  if (catch my $e) {
...
with non IO::Handle::Moose objects.
...
The B<print> method with EOL character at the end.
  ...
  $io = new IO::Handle::Moose fd=>\*STDOUT, mode=>"w";
  $io->say("Hello!");
...
an array in array context (like B<getlines> method).
  ...
  $io1 = new IO::Handle::Moose fd=>$f, "r";
  $passwd_file = $io1->slurp;
  
  $io2 = new IO::Handle::Moose fd=>$f, "r";
  $io2->autochomp(1);
  @passwd_lines = $io2->slurp;

=item IO::Handle::Moose->slurp(I<fd>)

Creates the B<IO::Handle::Moose> object and returns its content as a scalar in
scalar context or as an array in array context.

  open $f, "/etc/passwd";  
  $passwd_file = IO::Handle::Moose->slurp($f);
...
The IO::Handle::Moose object is stored in hash available via globref.
